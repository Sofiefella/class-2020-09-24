...
top_five_inc <- inner_join(sep_old, sep_new, by = "id", suffix = 
                             c("_old", "_new")) %>%
  mutate(enrollment_change = u_grad_new - u_grad_old) %>%
  arrange(desc(enrollment_change)) %>%
  select(name_new, department_new, enrollment_change) %>%
head(5) %>%
  gt() %>%
  tab_header(title = "Top 5 Classes with Highest Enrollment Increases") %>%
  cols_label(name_new = "Name",
...
top_five_dec <- inner_join(sep_old, sep_new, by = "id", suffix = 
                             c("_old", "_new")) %>%
  mutate(enrollment_change = u_grad_new - u_grad_old) %>%
...
head(5) %>%
  gt() %>%
  tab_header(title = "Top 5 Classes with Highest Enrollment Decreases") %>%
  cols_label(name_new = "Name",
             department_new = "Department",
             enrollment_change = "Change") 
top_five_dec
...
full_join(top_five_inc, top_five_dec, by = c("enrollment_change", 
                     "name_new", "department_new")) %>%
  arrange(enrollment_change) %>%
  mutate(name_new = reorder(name_new, enrollment_change)) %>%
  ggplot(aes(x = name_new, y = enrollment_change, fill = department_new)) + 
  geom_col() +
  coord_flip()
